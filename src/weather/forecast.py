...
def get_peak_profile(forecast_df: pd.DataFrame) -> Optional[str]:
    ...
    Returns a string like:
      "sharp peak at 14h, drops 5°F in 2h after peak"
      "flat peak 13h-16h (3h within 1°F), drops 2°F in 2h after"
    """
    if forecast_df.empty or "temperature_f" not in forecast_df.columns:
        return None

    temps = forecast_df["temperature_f"].values
    ts = pd.to_datetime(forecast_df["timestamp"])
    peak_val = float(max(temps))
    peak_idx = int(temps.argmax())
    peak_hour = ts.iloc[peak_idx].hour

    # Hours within 1°F and 2°F of peak
    near_1f = temps >= (peak_val - 1.0)
    near_2f = temps >= (peak_val - 2.0)
    hours_1f = int(near_1f.sum())
    hours_2f = int(near_2f.sum())
    ...
    # Range of hours within 1°F
    idx_1f = [i for i, v in enumerate(near_1f) if v]
    first_1f = ts.iloc[idx_1f[0]].hour
    last_1f = ts.iloc[idx_1f[-1]].hour

    idx_2f = [i for i, v in enumerate(near_2f) if v]
    first_2f = ts.iloc[idx_2f[0]].hour
    last_2f = ts.iloc[idx_2f[-1]].hour

    return (
        f"{shape} peak — {hours_1f}h within 1°F ({first_1f}h-{last_1f}h), "
        f"{hours_2f}h within 2°F ({first_2f}h-{last_2f}h)"
    )
